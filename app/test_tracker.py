...
def _mock_fetch(fail_ticker=None):
    ...
    def _fetch(symbol):
        ...
        return {"ticker": symbol, "price": 100.00, "change_pct": 1.50, "market_status": "open"}
    return _fetch
...
def test_market_summary_returns_json_array(client):
    ...
    for item in data:
        assert {"ticker", "price", "change_pct", "market_status"} <= item.keys()
...
def test_one_failed_ticker_still_returns_200(mocker):
    mocker.patch("tracker.fetch_ticker", side_effect=_mock_fetch(fail_ticker="^NSEI"))
    with app.test_client() as c:
...
def test_failed_ticker_has_error_field(mocker):
    ...
    failed = next(d for d in data if d["ticker"] == "^NSEI")
...
def test_other_tickers_unaffected_by_one_failure(mocker):
    ...
    with app.test_client() as c:
        data = json.loads(c.get("/").data)
    ok = [d for d in data if d["ticker"] != "^NSEI"]
